...
class SignUpPage extends BasePage {
  ...
  async verifyCreatePasswordScreenContent(): Promise<this> {
    ...
    await expect.soft(uppercaseEl)
      .toHaveText(CREATE_PASSWORD_SCREEN.requirementUppercase, { message: 'Uppercase requirement should match' });
    
    await expect.soft(lowercaseEl)
      .toHaveText(CREATE_PASSWORD_SCREEN.requirementLowercase, { message: 'Lowercase requirement should match' });
    
    await expect.soft(numberEl)
      .toHaveText(CREATE_PASSWORD_SCREEN.requirementNumber, { message: 'Number requirement should match' });
    
    await expect.soft(specialEl)
      .toHaveText(CREATE_PASSWORD_SCREEN.requirementSpecial, { message: 'Special character requirement should match' });
  ...
  async verifyCreatePasswordWarningText(): Promise<this> {
    await expect.soft(this.createPasswordWarningText)
      .toHaveText(CREATE_PASSWORD_SCREEN.warningText, { message: 'Warning text should match' });
    return this.self;
  }
  ...
  async tapTermsLink(): Promise<this> {
    try {
      const linkElement = this.createPasswordTermsLink;
      await linkElement.waitForDisplayed({ timeout: 3000 });
      await linkElement.click();
    } catch (error) {
      const fallbackElement = this.createPasswordTermsLinkFallback;
      await fallbackElement.waitForDisplayed({ timeout: 3000 });
      const location = await fallbackElement.getLocation();
      const size = await fallbackElement.getSize();
      const clickX = location.x + (size.width * 0.85);
  ...
  async verifyValidationError(
    ...
    field: 'enterPassword' | 'confirmPassword' | 'newVaultName' | 'enterMasterPassword' = 'enterPassword'
  ): Promise<this> {
    ...
    switch (errorType) {
      ...
      case 'passwordsDoNotMatch':
        if (field === 'confirmPassword') {
          ...
          throw new Error(`passwordsDoNotMatch error is only applicable for confirmPassword field`);
        }
        break;
    ...
    await expect.soft(errorElement)
      .toBeDisplayed({ message: `Validation error "${errorType}" should be displayed` });
    ...
    if (errorType === 'incorrectPassword') {
      ...
      await expect.soft(errorElement)
        .toHaveText(` ${expectedText} `, { message: `Validation error text should match "${expectedText}"` });
    }
  ...
  async verifyEnterPasswordScreenContent(): Promise<this> {
    ...
    await expect.soft(this.enterPasswordWarning)
      .toHaveText(ENTER_PASSWORD_SCREEN.warning, { message: 'Warning should match' });
  ...
  get selectVaultsVaultsList() { return this.$('selectVaultsVaultsList'); }
  ...
  async verifySelectVaultsTitle(): Promise<this> {
    ...
    await expect.soft(titleEl)
      .toHaveText(SELECT_VAULT_TYPE_SCREEN.listTitle, { message: 'Select Vaults title should match' });
    
    return this.self;
  }

  async tapSelectVaultsVaultItem(): Promise<this> {
    const vaultItem = this.selectVaultsVaultsList;
