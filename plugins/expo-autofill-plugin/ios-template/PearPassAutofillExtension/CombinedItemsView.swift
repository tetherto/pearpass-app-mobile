//
//  CombinedItemsView.swift
//  PearPassAutoFillExtension
//
//  combined vault + credentials screen. Mirrors android-template/res/layout/fragment_combined_items.xml.
...
struct CombinedItemsView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            PPSearchField(text: $searchText, placeholder: NSLocalizedString("Search in All Items", comment: "search placeholder"))
            ...
            PPContentCard {
                VStack(spacing: 0) {
                    ...
                    if mode == .registration {
                        ...
                        PPButton(
                            title: NSLocalizedString("Add New Login", comment: "add new login button"),
                            variant: .primary,
    ...
    private var currentVaultName: String {
        guard let id = selectedVaultId,
            ...
            return NSLocalizedString("Select Vault", comment: "vault picker placeholder")
        }
        return vault.name
    }
    ...
    private var emptyState: some View {
        VStack {
            switch mode {
            case .assertion:
                Text(NSLocalizedString("No matching items in this vault", comment: "empty state — assertion"))
    ...
    private var registrationEmptyText: some View {
        let highlight = Text(NSLocalizedString("save it as a new item", comment: "empty state — clickable highlight"))
            .foregroundColor(PPColors.primary)
            .underline()
        let prefix = Text(NSLocalizedString("We couldn't find a matching item in this vault. Would you like to ", comment: "empty state — registration prefix"))
        let suffix = Text(NSLocalizedString(" or look in another vault?", comment: "empty state — registration suffix"))
