...
//
//  Root SwiftUI view for the assertion + registration flows
...
//  to the view bindings.
//

import SwiftUI
import AuthenticationServices

/// iOS-version-agnostic registration context. Lets HostView (which targets all
...
struct HostView: View {
    ...
    /// remounts, body re-render, etc.). Cheaper than a singleton for the
    ...
    var registrationContext: RegistrationContext? = nil
    ...
    var body: some View {
        ZStack {
            ...
            if isInitializing {
                ...
                ErrorBoundaryView(
                    title: NSLocalizedString("Autofill Error", comment: "error screen title"),
                    subtitle: NSLocalizedString("Unable to initialize autofill", comment: "error screen subtitle"),
                    message: NSLocalizedString("An unexpected error occurred. Please try again.", comment: "error screen body"),
                    onBack: handleClose
                )
        ...
        .alert(
            ...
            Button(NSLocalizedString("Replace", comment: "replace passkey confirm button"), role: .destructive) {
    ...
    private var routedView: some View {
        ...
        case .missingConfiguration:
            MissingConfigurationView(onBack: handleClose)

        case .masterPassword:
            MasterPasswordView(
                headerTitle: mode == .registration
                    ? NSLocalizedString("Add a passkey", comment: "master password header — registration")
        ...
        case .vaultSelection, .vaultPassword, .credentialsList:
            CombinedItemsView(
                headerTitle: mode == .registration
                    ? NSLocalizedString("Create Passkey", comment: "sheet header — passkey registration")
                    : NSLocalizedString("Sign in", comment: "sheet header — credential assertion"),
                mode: mode,
    ...
    private var passkeyFormView: some View {
        PasskeyFormView(
            headerTitle: NSLocalizedString("Create Passkey", comment: "sheet header — passkey registration"),
    ...
    private func initializeUser(client: PearPassVaultClient) async {
        do {
            ...
            // directly to CombinedItemsView on the first vault.
    ...
    private func handleFaceIDLogin() {
        ...
        Task {
            do {
                ...
                NSLog("[HostView] Face ID unlock error: \(error)")
                await MainActor.run {
                    isAuthenticating = false
    ...
    private func handlePasskeyLogin() {
        ...
        Task {
            do {
                ...
                NSLog("[HostView] Passkey unlock error: \(error)")
                await MainActor.run {
                    isAuthenticating = false
                    let nsError = error as NSError
                    if nsError.code == -128 {
    ...
    private func handleSelectCredential(_ id: String) {
        ...
        // rows through onComplete; This dispatches by record type instead of
    ...
    private func handleFormSave() {
        guard let ctx = registrationContext,
            ...
            formSaveError = NSLocalizedString("Missing registration context", comment: "passkey save error")
            return
        }
        guard !formTitleText.trimmingCharacters(in: .whitespaces).isEmpty else {
            formSaveError = NSLocalizedString("Title is required", comment: "passkey save error")
            return
        }
        ...
        Task {
            do {
                ...
                NSLog("[HostView] Save error: \(error)")
                await MainActor.run {
                    isSavingPasskey = false
                    formSaveError = NSLocalizedString("Failed to save passkey", comment: "passkey save error")
                }
            }
        }
    }
    ...
    private func generatePasskey(ctx: RegistrationContext) throws -> (PasskeyCredential, Data, Data) {
    ...
    private func loadCredentials(for vault: Vault) {
        ...
        Task {
            ...
            do {
                ...
                NSLog("[HostView] loadCredentials error: \(error)")
                await MainActor.run {
                    credentials = []
                    isLoadingCredentials = false
                }
    ...
    private func loadPendingPasskeysFromJobs(
        ...
        guard let hashedPassword = await PasskeyJobCreator.getHashedPassword(from: vaultClient) else {
            NSLog("[HostView] Could not get hashed password for job file decryption")
            return []
        }
        ...
        do {
            ...
            NSLog("[HostView] Failed to read pending jobs: \(error.localizedDescription)")
            return []
        }
    }
    ...
    private func passkeyCredentialFromJobPayload(
        ...
        guard let privateKeyData = Data(base64URLEncoded: privateKey) else {
            NSLog("[HostView] Failed to decode private key from job payload")
            return nil
        }
    ...
    /// passkey rows xor password rows (never mixed). This funnels everything
